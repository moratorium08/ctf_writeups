...
contract Ballot is ERC721, Ownable {
    ...
    event DebugIsResident(address indexed _address, string log);

    constructor(address owner) ERC721("BeginnersBallot", "BB") Ownable(owner) {}

    function registerAsResident(address person) public onlyOwner {
        emit DebugIsResident(person, "registerAsResident");
        isResident[person] = true;
    }

    function issueBallot() public returns (uint256) {
        emit DebugIsResident(msg.sender, "issueBallot");
        emit DebugIsResident(address(this), "issueBallot");
